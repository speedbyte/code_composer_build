...
add method addpads(), update() 27.10.07 Hajo Butz 
...
class IntelHex:
    ...
    def addpads(self, start=None, end=None, pad=None):
        ''' Add pads
        ...
        if start > end:
            start, end = end, start
        for i in xrange(start, end+1):
            self._buf[i] = self._buf.get(i, pad)
        return True

    def updateheximage(self, image=None):
        ''' Update hex-image with a other hex-image
        @param  image   hex-image that has to update other hex-image.
        @return True    if successful.
        '''
        if image is None:
            return False
        self._buf.update(image._buf)
        return True
            ...
            for addr in rng:
                ...
                if byte != None:
                    if k == 0:
                        ...
                        fobj_write(offset_record)
                        offset_record = ''
                        # start data record
##                        record += "%04X00" % addr
                        record = "%s%04X00" % (record,addr)
